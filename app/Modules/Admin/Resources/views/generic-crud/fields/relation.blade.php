{{--
    Relation field: a select populated from an Eloquent relation.
    Expects $field['model'], $field['value_field'], $field['label_field'].
--}}
@php
    $value   = old($field['name'], $record?->{$field['name']} ?? '');
    $options = \App\Services\ModuleCrudService::resolveRelationOptions($field);
@endphp
<div>
